...
public class ResetPaswordController {
  ...
  // TODO SEND EMAIL
  private ResponseEntity<MessageResponse> handleResetToken(ItsUser user){
  ...
  ResponseEntity<MessageResponse> resetPassword(@Valid @RequestBody ResetPasswordRequest request){
    try {
      ...
      ItsUser user = userRepository.findByIdAndPasswordResetToken(userId,passwordResetToken)
        ...
        return ResponseEntity.ok(new MessageResponse("Your new password has been saved please use your new password when you login to."));
    }
